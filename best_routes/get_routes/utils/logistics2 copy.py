    ...
    if (current_litres_in_tank <= min_litres and is_no_location_error):
      ...
      if (location):
        location_gas_stations = list(filter(lambda station: (
          station[3].strip().lower() in location['town'].lower() and (
            station[4].strip().lower() == location['state'].lower()
          )
        ), gas_stations_raw))
        ...
        if (len(location_gas_stations) > 0):
          ...
          current_litres_to_buy = current_litres_to_buy0 if (current_litres_to_buy0 <= max_litres_in_tank) else (
            max_litres_in_tank
          )
          gas_stations.append({
            'info': {
